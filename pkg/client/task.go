...
func (c *Client) FetchTasks(ctx context.Context, token string) ([]types.Task, error) {
	...
	// 2. 获取维度列表
	dimBody, err := c.doRequest(ctx, http.MethodGet,
		c.bizURL("/api/studentCircleNew/getDimensions"),
		nil, headers, "",
	)
	if err != nil {
		return nil, fmt.Errorf("FetchTasks getDimensions 请求失败: %w", err)
	}

	dimResp, err := types.DecodeResponse(dimBody)
	if err != nil {
		return nil, fmt.Errorf("FetchTasks getDimensions 响应解析失败: %w", err)
	}

	if err := types.CheckCode(dimResp); err != nil {
		return nil, fmt.Errorf("FetchTasks getDimensions 业务错误: %w", err)
	}

	dimensions, err := types.DecodeDataList[types.Dimension](dimResp)
	if err != nil {
		return nil, fmt.Errorf("FetchTasks 维度列表解析失败: %w", err)
	}
...
func (c *Client) GetDimensions(ctx context.Context, token string) ([]types.Dimension, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("GetDimensions 请求失败: %w", err)
	}

	resp, err := types.DecodeResponse(bodyBytes)
	if err != nil {
		return nil, fmt.Errorf("GetDimensions 响应解析失败: %w", err)
	}

	if err := types.CheckCode(resp); err != nil {
		return nil, fmt.Errorf("GetDimensions 业务错误: %w", err)
	}

	dimensions, err := types.DecodeDataList[types.Dimension](resp)
	if err != nil {
		return nil, fmt.Errorf("GetDimensions 维度列表解析失败: %w", err)
	}
