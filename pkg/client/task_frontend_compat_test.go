...
func TestSubmitTask_TargetName1_Compat(t *testing.T) {
	...
	if err != nil { t.Fatalf("target1 submit: %v", err) }
	if cap.Name != "活动名称1" || cap.Address != "操场" || cap.PlayRole != "2" { t.Fatalf("target1 field mismatch: %+v", cap) }
	if cap.Hours != 2 { t.Fatalf("target1 hours want 2 got %v", cap.Hours) }
}
...
func TestSubmitTask_TargetName4_ArtCompat(t *testing.T) {
	...
	if err != nil { t.Fatalf("target4 submit: %v", err) }
	if cap.Name == "" || cap.HostName != "校团委" || cap.ObtainTime != "2026-04-15" { t.Fatalf("target4 mismatch: %+v", cap) }
	if cap.Rank != "一等奖" || cap.Level != "5" { t.Fatalf("target4 rank/level mismatch: %+v", cap) }
	// hours preset>0 时可空，但本例 meta=4 且用户未显式 hours -> 用 preset
	if cap.Hours != 4 { t.Fatalf("target4 hours preset want 4 got %v", cap.Hours) }
}
...
func TestSubmitTask_TargetName6_Practice120h(t *testing.T) {
	...
	if err != nil { t.Fatalf("target6 submit: %v", err) }
	if cap.OrgName != "XX社区" || cap.Address != "XX社区" || cap.CheckResult != "1" { t.Fatalf("target6 mismatch: %+v", cap) }
	if cap.Hours != 120 { t.Fatalf("target6 hours want 120 got %v", cap.Hours) }
}
...
func TestSubmitTask_TargetName6_HoursOverride(t *testing.T) {
	...
	if err != nil { t.Fatalf("override: %v", err) }
	if cap.Hours != 80 { t.Fatalf("override hours want 80 got %v", cap.Hours) }
}
...
func TestSubmitTask_AllTargetNames_Smoke(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T){
			...
			if err != nil { t.Fatalf("%s submit: %v", tc.name, err) }
			if cap.Content != "c" { t.Fatalf("content lost") }
			if tc.check != nil { if err:=tc.check(cap); err!=nil { t.Fatal(err) }}
		})
	}
}
...
func _goldenDecodeTaskSubmitInput(data []byte) (types.TaskSubmitInput, error) {
	...
	if err := json.Unmarshal(data, &fields); err != nil { var in types.TaskSubmitInput; return in, err }
	for _, name := range []string{"hours","level","checkResult","playRole"} {
		...
		var num json.Number
		if err := json.Unmarshal(raw, &num); err != nil { var in types.TaskSubmitInput; return in, err }
		enc, _ := json.Marshal(num.String())
		fields[name] = enc
		// 同步大小写键：若原键是 Level，删除后用小写
		for k := range fields { if k != name && len(k)==len(name) { /* 大小写兼容已由 CLI 层处理，这里 golden 用小写即可 */ } }
	}
	// 处理 Level 大小写：若存在大写键则搬运
	for _, canon := range []string{"level","hours","checkResult","playRole"} {
		for k, v := range fields {
			if len(k)!=len(canon) { continue }
			if k==canon { continue }
			ok := true; for i:= range k { a:=k[i]; b:=canon[i]; if a>='A'&&a<='Z' { a+=32 }; if b>='A'&&b<='Z' { b+=32 }; if a!=b { ok=false; break } }
			if ok { fields[canon]=v; delete(fields,k) }
		}
	}
	norm, _ := json.Marshal(fields)
	var in types.TaskSubmitInput
	if err := json.Unmarshal(norm, &in); err != nil { return in, err }
	return in, nil
}
func TestSubmitTask_GoldenFrontendPractice120h(t *testing.T) {
	...
	defer c.Close()
	_, err = c.SubmitTask(t.Context(), "tok", in)
	if err != nil { t.Fatalf("golden practice submit: %v", err) }
	b, _ := json.Marshal(cap)
	var m map[string]any
	if err := json.Unmarshal(b, &m); err != nil { t.Fatalf("marshal: %v", err) }
...
func TestSubmitTask_GoldenFrontendArtSinging(t *testing.T) {
	...
	in, err := _goldenDecodeTaskSubmitInput(raw)
	if err != nil { t.Fatalf("decode art: %v", err) }
	if in.Level != "5" || in.Hours != "4" { t.Fatalf("art numeric->string: %+v", in) }
	var cap types.TaskAddCirclePayload
	srv := mockBizWithMetaAndCapture(t, metaForTask(4,4), &cap)
	defer srv.Close()
	c, _ := New(WithBaseURL(srv.URL), WithSSOBase(srv.URL), WithTimeout(5*1000*1000*1000))
	defer c.Close()
	_, err = c.SubmitTask(t.Context(), "tok", in)
	if err != nil { t.Fatalf("art submit: %v", err) }
...
func TestParseHours_ZeroAllowedForNonRequiredTarget(t *testing.T) {
	if _, err := parseHours("", 0, 2); err != nil { t.Fatalf("target 2 hours 0 should be allowed: %v", err) }
	if _, err := parseHours("", 0, 4); err != nil { t.Fatalf("target 4 hours 0 should be allowed: %v", err) }
	if _, err := parseHours("", 0, 6); err == nil { t.Fatal("target 6 hours 0 should be required") }
	if _, err := parseHours("", 0, 1); err == nil { t.Fatal("target 1 hours 0 should be required") }
	if _, err := parseHours("", 0, 10); err == nil { t.Fatal("target 10 hours 0 should be required") }
}
...
func TestEditCircle_TargetName4And6(t *testing.T) {
	...
	if err != nil { t.Fatalf("edit 4: %v", err) }
	if cap.ID == nil || *cap.ID != 5400001 { t.Fatalf("edit id not carried: %+v", cap.ID) }
	...
	if err != nil { t.Fatalf("edit 6: %v", err) }
	if cap2.Hours != 120 || cap2.OrgName != "o2" { t.Fatalf("edit 6 cap: %+v", cap2) }
}
...
func TestSubmitTask_PresetHoursZeroRequiresExplicit(t *testing.T) {
	...
	if err == nil { t.Fatal("meta 0 且未填 hours 应失败") }
}
