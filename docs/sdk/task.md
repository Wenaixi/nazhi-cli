...
> 等级常量：`types.TaskLevelNational/Province/City/County/School/Grade`（"1".."6"）与 `TaskLevelName(code)`、`CheckResultName(code)` 可用于可读赋值与展示，对齐前端原生 `src` 展示映射；真实字典以 `GET /api/common/sys/dict/list?cateCode=23` 为准。  
