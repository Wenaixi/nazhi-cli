...
## 架构深化候选 (v0.4.0 实施 #1~#5)

v0.3.5→v0.4.0 期间通过 `arch-tdd` 流程识别了 8 个架构深化候选，前 5 项已落地：
...
| #4 | **tokenparse 包** | **新包** `pkg/tokenparse/tokenparse.go`（v0.4.0 关键交付） | `auth.go` 原内嵌的 `extractTokenFromLocation` / `extractTokenFromReturnData` 抽到独立包，附带 `DefaultTokenTTL=24h` 兜底、移除 `ErrLocationParseFailed` 死 sentinel |
...
│   │   ├── errors.go            # 哨兵错误 (10 个)
│   │   └── *_test.go            # 单元测试 (~40 文件)
│   ├── tokenparse/              # v0.4.0 新增 — SSO token 解析
...
> v0.4.0 新增：Windows DLL 占用降级 + 启动清扫历史 temp 目录 — 详见 `docs/cross-platform-ocr.md`。
...
**v0.4.0 收口**：原散落的 4 处状态字段被收口到 `pkg/client/session.go` 的 `sessionManager` 状态机
...
### 8.5. token 解析下沉到 pkg/tokenparse（v0.4.0）

`auth.go` 原内嵌的 `extractTokenFromLocation` / `extractTokenFromReturnData` 抽到
新包 `pkg/tokenparse/tokenparse.go`（架构深化候选 #4 关键交付）：
...
pkg/client  ← Option 模式 + 13 个公开方法 + sessionManager
        ↓
pkg/tokenparse  ← SSO token 解析 (v0.4.0 新增)
        ↓
...
用户 → nazhi login
     ...
     → errgroup.WithContext:                     ← v0.4.0 F5：无数据依赖，并发
