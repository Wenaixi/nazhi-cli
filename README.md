...
**输出通道**：`stdout` 只承载成功数据，**错误一律写 `stderr`**。无论成功或失败，退出码语义完全一致——
脚本判成败请读退出码或 `stdout` 的 `status` 字段，读错误详情请读 `stderr`。

> **v1.8.0 变更**：此前参数错误（退出码 3）会同时出现在 `stdout`，从 v1.8.0 起统一只写 `stderr`。
> 依赖旧行为用 `nazhi ... 2>/dev/null | jq` 捕获参数错误的脚本，请改为 `nazhi ... 2>&1 >/dev/null` 或直接读 `stderr`。
> 退出码、`code` 字段与错误文案均未改变。
