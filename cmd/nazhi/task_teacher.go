...
支持 --limit / --offset 分批拉取，--count 只看总数，--key 关键字筛选。`,
  ...
  nazhi task teacher --key 劳动                           # 按关键字筛选`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		// 会绕过 offset/limit 校验静默返回 total，脚本拿错形状不自知。
		...
		if offset > 0 || limit > 0 {
			...
			if err != nil {
				if len(raw) > 0 {
					...
					printEnvelope(envelope.Partial(207, "获取教师写实记录失败: "+err.Error(), map[string]any{
		...
		if err != nil {
			if len(raw) > 0 {
				printEnvelope(envelope.Partial(207, "获取教师写实记录失败: "+err.Error(), json.RawMessage(raw)))
				return
			}
			printError(fmt.Errorf("获取教师写实记录失败: %w", err))
			return
