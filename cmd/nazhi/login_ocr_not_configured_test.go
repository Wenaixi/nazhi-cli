...
// 而非通用 printError。
// 测试策略：模拟 loginCmd.Run 的 err 分支，验证 stdout envelope 输出包含中文指引。
...
func TestLoginCmd_ErrOCRNotConfigured_ActionableOutput(t *testing.T) {
	...
	if errors.Is(err, client.ErrOCRNotConfigured) {
		printEnvelope(envelope.Error(503, "登录失败：OCR 识别器未配置。当前构建未启用 -tags ddddocr，无法自动识别验证码。请下载预编译 release 二进制或注入自定义识别器。"))
	}
	...
	// stdout 应为 envelope（status=error, code=503）
	if !strings.Contains(stdout, `"status": "error"`) {
		t.Errorf("stdout 应包含 status=error，实际: %s", stdout)
	}
	if !strings.Contains(stdout, `"code": 503`) {
		t.Errorf("stdout 应包含 code=503（OCR 不可用），实际: %s", stdout)
	}
	if !strings.Contains(stdout, "OCR 识别器未配置") {
		t.Errorf("stdout 应包含中文指引，实际: %s", stdout)
	}
}
