...
var taskListCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			// F9 修复（round-7）：区分「全失败」与「部分失败」。
			//
			// SDK 设计契约（T1 round-5）：FetchTasks 在「单维度失败但其他维度成功」时
			...
			// F11 修复（group-F round-8）：扩展 envelope 分支覆盖 ErrEmptyUserInfo。
			//
			// round-7 F9 只覆盖 ErrBusinessRejected，但当 session 预热步骤 4
