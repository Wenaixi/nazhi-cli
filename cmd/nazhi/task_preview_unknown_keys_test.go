...
// ：task preview 与 task submit/edit 同族入口必须共用未知键白名单。
...
// TestTaskPreview_UnknownTopLevelKey_Rejects 锁定 ：task preview 含未知顶层键
...
// TestTaskPreviewEdit_UnknownTopLevelKey_Rejects 锁定 ：task preview --edit 同款拒绝。
