	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := pendingExitCode.Load(); got != 3 {
				t.Errorf("单独 --offset 应拒绝为参数错误(退出码 3), 实际 %d; stderrstdout=%q", got, stderrstdout)
			}
			if !strings.Contains(stderrstdout, "--offset") {
...
func TestTaskCirclesCommands_RejectNegativeOffset(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := pendingExitCode.Load(); got != 3 {
				t.Errorf("负数 --offset 应拒绝为参数错误(退出码 3), 实际 %d; stderrstdout=%q", got, stderrstdout)
			}
			if !strings.Contains(stderrstdout, "--offset") {
				t.Errorf("stderrstdout 应含参数错误提示, 实际: %q", stderrstdout)
			}
