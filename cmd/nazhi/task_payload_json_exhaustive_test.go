...
func TestDecodeTaskInputJSON_Exhaustive_NumericHours(t *testing.T) {
	cases := []struct{ name, payload string; wantHours string; wantErr bool }{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantErr {
				if err == nil { t.Fatalf("want err") }
				return
			}
			if err != nil { t.Fatalf("unexpected %v", err) }
			if in.Hours != tc.wantHours { t.Fatalf("want %q got %q", tc.wantHours, in.Hours) }
		})
	}
}
...
func TestDecodeTaskInputJSON_Exhaustive_DiscreteCodes(t *testing.T) {
	for _, field := range []string{"level", "checkResult", "playRole"} {
		t.Run(field+" int ok", func(t *testing.T) {
			...
			if err := decodeTaskInputJSON(payload, &in); err != nil { t.Fatalf("%v", err) }
		})
		t.Run(field+" 1.0 canonicalizes to 1", func(t *testing.T) {
			...
			if got != "1" { t.Fatalf("want 1 got %q", got) }
		})
		...
		t.Run(field+" 5.5 reject", func(t *testing.T) {
			...
			var in types.TaskSubmitInput
			if err := decodeTaskInputJSON(payload, &in); err == nil { t.Fatal("should reject fractional") }
		})
		t.Run(field+" 1e999 reject non-finite", func(t *testing.T) {
			payload := []byte(`{"taskId":1,"content":"c","` + field + `":1e999}`)
			var in types.TaskSubmitInput
			if err := decodeTaskInputJSON(payload, &in); err == nil { t.Fatal("should reject non-finite") }
		})
		t.Run(field+" -0.0 -> 0", func(t *testing.T) {
			...
			if got != "0" { t.Fatalf("want 0 got %q", got) }
		})
		t.Run(field+" string stays", func(t *testing.T) {
			...
			if got != "5" { t.Fatalf("want 5 got %q", got) }
		})
		...
		t.Run(field+" bool reject", func(t *testing.T) {
			...
			var in types.TaskSubmitInput
			if err := decodeTaskInputJSON(payload, &in); err == nil { t.Fatal("bool should reject") }
		})
		t.Run(field+" case-insensitive LEVEL", func(t *testing.T) {
			...
			if got != "3" { t.Fatalf("want 3 got %q for %s", got, upper) }
		})
	}
}

func TestDecodeTaskInputJSON_Exhaustive_Aliases(t *testing.T) {
	cases := []struct{ name, payload string; wantTaskID int64; wantImageIDs []int64 }{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if in.TaskID != tc.wantTaskID { t.Fatalf("want taskId %d got %d", tc.wantTaskID, in.TaskID) }
			if tc.wantImageIDs != nil {
				if len(in.ImageIDs) != len(tc.wantImageIDs) { t.Fatalf("want %v got %v", tc.wantImageIDs, in.ImageIDs) }
				for i, v := range tc.wantImageIDs {
					if in.ImageIDs[i] != v { t.Fatalf("want %v got %v", tc.wantImageIDs, in.ImageIDs) }
				}
			}
		})
	}
...
func TestDecodeTaskInputJSON_Exhaustive_OtherFieldsPreserved(t *testing.T) {
	...
	if err != nil { t.Fatalf("%v", err) }
	if in.Name != "活动" || in.HostName != "校" || in.Address != "操场" || in.Rank != "一等" { t.Fatalf("basic string not preserved %+v", in) }
	if in.ObtainTime != "2026-04-15" || in.TeamName != "t" || in.PatentNum != "pn" { t.Fatalf("extended fields not preserved %+v", in) }
	if in.LikeSpecialty3 != "c" || in.SpecialtyTechnology != "tech" { t.Fatalf("like/tech not preserved %+v", in) }
}
...
func TestDecodeTaskInputJSON_Exhaustive_IdField(t *testing.T) {
	...
	if err != nil { t.Fatalf("%v", err) }
	if in.ID != 5400001 { t.Fatalf("want 5400001 got %d", in.ID) }
...
func TestDecodeTaskInputJSON_Exhaustive_InvalidJSON(t *testing.T) {
	if _, err := decodeTaskSubmitInput([]byte(`{"taskId":}`)); err == nil { t.Fatal("invalid json should fail") }
	if _, err := decodeTaskSubmitInput([]byte(`not json`)); err == nil { t.Fatal("not json should fail") }
	// top-level array should fail (we marshal to map)
	if err := decodeTaskInputJSON([]byte(`[1,2]`), &types.TaskSubmitInput{}); err == nil { t.Fatal("array top should fail") }
}
...
func TestDecodeTaskInputJSON_Exhaustive_GoldenFrontendJSON(t *testing.T) {
	...
	if err != nil { t.Fatalf("practice golden decode %v", err) }
	if in.Hours != "120" { t.Fatalf("want 120 got %q", in.Hours) }
	if in.OrgName != "社区" || in.Address != "基地" { t.Fatalf("golden org/address %+v", in) }
	b, _ := json.Marshal(in)
	_ = b
	...
	if err != nil { t.Fatalf("art golden %v", err) }
	if in2.Level != "5" || in2.Hours != "4" { t.Fatalf("art want 5/4 got %q/%q", in2.Level, in2.Hours) }
	// canonical taskId should win over circleTaskId alias
	if in2.TaskID != 1002 { t.Fatalf("canonical taskId should win got %d", in2.TaskID) }
}
...
func TestNormalizeTaskInputJSON_Exhaustive_BigIntAndOverflow(t *testing.T) {
	...
	var in types.TaskSubmitInput
	if err := decodeTaskInputJSON(payload, &in); err != nil { t.Fatalf("big int should be allowed, got %v", err) }
	if in.Level != "9223372036854775808" { t.Fatalf("want big int string got %q", in.Level) }
	...
	if err := decodeTaskInputJSON(payload2, &in2); err == nil { t.Fatal("fractional big should reject") }
}
